...
// define the pin usage
  //analogWriteFrequency(MTR_R_PWM, pwm_freq);
  //analogWriteFrequency(MTR_L_PWM, pwm_freq);
  analogWriteResolution(ANL_WR_LEN);  // set up the PWM resolution
// right motor pins
...
// right motor speed control  [NOTE: may consider in the future disabling interrupts briefly here to prevent a right motor adjustment and a delayed left motor adjustment]
  ...
  analogWrite(MTR_R_PWM, abs(speed_r));     // Set the PWM for the right motor
  ...
  analogWrite(MTR_L_PWM, abs(speed_l));     // Set the PWM for the left motor
}
