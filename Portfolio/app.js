...
const WEEK_TOPICS = {
    ...
    12: { title: 'L4.1 Microcontroller Fundamentals + L4.2 Variables & Types', phase: 'programming', unit: '04', focus: 'L4.1 Microcontroller Fundamentals + A4.1 Blink; L4.2 Variables & Types + A4.2' },
    13: { title: 'L4.3 Functions + D4.1', phase: 'programming', unit: '04', focus: 'L4.3 Python Functions + A4.3 SOS with Functions. Short week — Thanksgiving Mon–Tue only. D4.1 due end of week.' },
...
const DELIVERABLES = [
    ...
    // ── Unit 4: Programming Basics — D4.1 (Lessons 4.1–4.3) ────────────
    {
        ...
        requirements: [
            ...
            '4.2 Q1: Name the four primitive data types used most often in MicroPython (int, float, bool, str). Give one concrete example value from the robot for each type.',
            '4.2 Q2: What is the difference between x = 5 and x = 5.0 in MicroPython? Describe one situation in a robot program where using the wrong type causes a bug.',
            '4.2 Q3: The expression duty = percent / 100 * 65535 produces a float, but duty_u16() expects an integer. Rewrite the expression using int() and explain why the conversion is required.',
            '4.2 Q4: A student writes speed = "75" and then passes it to set_speed(speed). What goes wrong, and how do you fix the assignment so speed holds the correct type?',
            '4.2 Q5: Identify and write the corrected lines for the two bugs in the debug snippet from Activity 4.2.',
            '4.3 Q1: What Python keyword starts every function definition?',
            '4.3 Q2: In def blink(duration_ms):, what is duration_ms? When you write blink(400), what value does duration_ms hold while the function runs?',
            '4.3 Q3: What does DRY stand for? Describe one specific example from Activity 4.3 where using a function let you follow the DRY principle.',
            '4.3 Q4: Can a function call another function? Trace the chain of calls that happens when your code executes sos() — list every function that runs and in what order.',
    ...
    // ── Unit 4: Programming Basics — D4.2 (Lessons 4.4–4.6) ────────────
    {
        ...
        requirements: [
            '4.4 Q1: What does sw2.value() return when SW_2 is not pressed? When pressed? Why those specific values?',
            '4.4 Q2: Why can\'t you configure a pin as Pin.IN without also setting a pull resistor? What happens if you leave it out?',
            '4.4 Q3: SW_2 uses Pin.PULL_DOWN. If you changed it to Pin.PULL_UP, what would you need to change in the if statement to keep the same behavior? Why?',
            '4.4 Q4: The LED stays on all the time and nothing changes when you press the switch. List the three most likely causes you would check first.',
            '4.4 Q5: Identify and write the corrected lines for the two bugs in the debug snippet from Activity 4.4.',
            '4.5 Q1: What does PWM stand for? Explain how rapid on/off switching simulates lower power without changing voltage.',
            '4.5 Q2: duty_u16() accepts values 0–65535. What value gives 75% duty cycle? Show your calculation.',
            '4.5 Q3: What does freq() control in a PWM signal? Would you notice a difference between freq(10) and freq(1000) on an LED? Explain.',
            '4.5 Q4: Write the line inside set_brightness(percent) that calculates the duty value. Why is int() required?',
            '4.5 Q5: Identify the two bugs in the debug snippet from Activity 4.5 and write the corrected version of each line.',
